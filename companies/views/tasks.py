...
from companies.serializers import TaskSerializer, TasksSerializers
...
class Tasks(Base):
    ...
    def post(self, request):
        ...
        if not title or len(title) > 125:
            raise APIException("Envie um título válido para essa tarefa. Título auxente ou maior que 125 caracteres.")
...
class TaskDetail(Base):
    ...
    def put(self, request, task_id):
        ...
        if not serializer.is_valid():
            raise APIException("Dados Inválidos! Não foi possível atualizar a Task.")
        
        serializer.update(task, serializer._validated_data)
